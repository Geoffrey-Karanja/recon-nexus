...
export default function ScanView({ scanId, onBack }: Props) {
  ...
  const tools = ['whois', 'subfinder', 'theHarvester', 'dnsx', 'nmap', 'httpx', 'wafw00f']
  const statusColor: Record<string, string> = {
    queued: 'var(--text-dim)', running: 'var(--cyan)', done: 'var(--green)', error: 'var(--red)'
  }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <header style={{
        ...
        display: 'flex', alignItems: 'center', gap: 16, background: 'var(--bg2)', flexShrink: 0,
      }}>
        ...
        <span style={{
          ...
          color: scan?.status === 'running' ? 'var(--cyan)' : scan?.status === 'done' ? 'var(--green)' : 'var(--text-dim)',
          padding: '2px 8px', borderRadius: 3,
          border: `1px solid ${scan?.status === 'running' ? 'var(--cyan)' : scan?.status === 'done' ? 'var(--green)' : 'var(--border)'}`,
          animation: scan?.status === 'running' ? 'pulse-green 1.5s infinite' : 'none',
        }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Left sidebar — tool pipeline */}
        <div style={{
          width: 180, borderRight: '1px solid var(--border)', background: 'var(--bg2)',
          display: 'flex', flexDirection: 'column', flexShrink: 0,
        }}>
          <div style={{ padding: '12px 16px', fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-dim)', letterSpacing: 2, borderBottom: '1px solid var(--border)' }}>
            PIPELINE
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
            {tools.map(tool => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: 10, color: statusColor[st] ?? 'var(--text-dim)', animation: st === 'running' ? 'pulse-green 1s infinite' : 'none' }}>
                    {statusIcon[st] ?? '○'}
                  </span>
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: isActive ? 'var(--cyan)' : 'var(--text-dim)' }}>
                    {tool}
                  </span>
                </div>
              )
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg2)', flexShrink: 0 }}>
            {(['graph', 'terminal', 'findings'] as const).map(tab => (
              <button
                ...
                style={{
                  padding: '10px 24px', cursor: 'pointer', background: 'none',
          ...
          <div style={{ flex: 1, overflow: 'hidden' }}>
            ...
            {activeTab === 'findings' && (
              <div style={{ height: '100%', overflowY: 'auto', padding: 20 }}>
                {findings.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '60px', fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--text-dim)' }}>
                    AWAITING FINDINGS...
                  </div>
                ) : (
